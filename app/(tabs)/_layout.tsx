...
// --- NOVOS IMPORTS (Apenas Lógica) ---
import { signOut } from "firebase/auth";
import { auth } from "../../services/firebaseConfig"; // Ajuste o caminho se necessário
import { useAuth } from "../contexts/auth"; // Importando o contexto que criamos

// --- SEU COMPONENTE CUSTOMIZADO (Design Intacto) ---
function CustomDrawerContent(props: DrawerContentComponentProps) {
  // Caminho da imagem (Ajuste o caminho se necessário)
  const LOGO_PATH = require("../assets/logo.png");

  // --- LÓGICA CONECTADA ---
  // Pegamos o usuário real do contexto
  const { user } = useAuth();
  // Se tiver usuário, mostra o nome dele. Se não, mostra "Visitante" (ou 'Davi' se preferir manter fixo enquanto carrega)
  const USER_NAME = user?.displayName || "Cliente";

  const handleLogout = async () => {
    try {
      console.log("[AUTH] Fazendo logout...");
      await signOut(auth); // Desloga do Firebase
      router.replace("/login"); // Manda para a tela de login
    } catch (error) {
    ...
    <View style={drawerStyles.container}>
      ...
      <View style={drawerStyles.header}>
        <View style={drawerStyles.profileContainer}>
          <Image source={LOGO_PATH} style={drawerStyles.profileImage} />
      ...
      >
        ...
        >
          <View style={drawerStyles.logoutRow}>
            ...
            {/* Removi o onPress do Text para não dar conflito com o TouchableOpacity pai */}
    ...
    >
      ...
      <Drawer.Screen
        ...
        options={{
          ...
          // href: null, // Comentado para não sumir o header se precisar navegar
          drawerIcon: ({ color, size }) => (
...
// --- SEUS ESTILOS --- (Mantidos 100% iguais)
